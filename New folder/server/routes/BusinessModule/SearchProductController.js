        ...
        async.waterfall([
            ...
            function search(data, cb) {
                ...
                if (req.body.minPrice) {
                    let reverse = 1;
                    try {
                        reverse = parseFloat(req.body.minPrice * data.reverse);
                        // currencyQuery = ', toFloat(b.priceInUSD) * ' + data.reverse + ' AS price, "' + currency + '" AS currency ';
                    } catch (exception) {
                ...
                if (req.body.maxPrice) {
                    let reverse = 1;
                    try {
                        reverse = parseFloat(req.body.maxPrice * data.reverse);
                        // currencyQuery = ', toFloat(b.priceInUSD) * ' + data.reverse + ' AS price, "' + currency + '" AS currency ';
                    } catch (exception) {
    ...
    Router.post('/searchFilter/m', (req, res) => {
        ...
        async.waterfall([
            ...
            function search(data, cb) {
                ...
                if (req.body.minPrice) {
                    query += ' AND toFloat(b.priceInUSD) >= ' + parseFloat(req.body.minPrice * data.reverse) + ' ';
                }
                if (req.body.maxPrice) {
                    query += ' AND toFloat(b.priceInUSD) <= ' + parseFloat(req.body.maxPrice * data.reverse) + ' ';
                }
                if (req.body.postedWithin) {
                    var time = parseInt(req.body.postedWithin);
                    var lala = moment().subtract(time, 'days').valueOf();
