...
function googleSignupToken(user, accessKey) {
...
function registrationToken(user, accessKey) {
    var token = jsonwebtoken.sign({
        id: user[0].userId,
        name: user[0].username,
        accessKey: accessKey
        ...
        function register(userData, callback) {
            ...
            dbneo4j.cypher({
                ...
                if (e) {
                    ...
                    var authToken = googleSignupToken(d, accessKey);
        ...
        function register(callback) {
            ...
            dbneo4j.cypher({
                ...
                if (e) {
                    ...
                    var authToken = registrationToken(d, accessKey);
...
authServices.googleLogin = (userData, cb) => {
    ...
    async.waterfall([
        ...
        function sendResponse(d, callback) {
            ...
            var token = registrationToken(d, accessKey);
            var userCollection = mongoDb.collection('user');
            userCollection.update({
    ...
    function googleEmailLogin(callback) {
        var query = `MATCH (a : User) WHERE (a.reject <> 1 OR NOT EXISTS(a.reject)) AND a.email = "` + userData.email + `" ` +
            `SET a.lastLogin = ` + moment().valueOf() + `, a.facebookId = "` + userData.googleId + `", a.googleVerified = ` + 1 + ` ` + condition +
