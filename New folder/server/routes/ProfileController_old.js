...
module.exports = function (app, express) {
        ...
        async.waterfall([
            function userProfile(cb) {
                var query = `MATCH (a : User {username : "` + username + `"}) WITH a ` +
                    ...
                    `RETURN DISTINCT a.phoneNumber AS phoneNumber, a.profilePicUrl AS profilePicUrl, a.fullName AS fullName, ` +
            ...
            // function sellingItems(profileData, cb) {
            ...
            //         "WITH n, p, posts, l, user, allLikes, node3, c " +
            ...
            //         "posts.postCaption AS postCaption, posts.imageCount AS imageCount, " +
            //         "p.type AS postsType, p.postedOn AS postedOn, posts.latitude AS latitude, posts.longitude AS longitude, posts.productsTaggedCoordinates AS productsTaggedCoordinates, " +
            ...
            //         "posts.productUrl AS productUrl, posts.title AS title, posts.description AS description, posts.negotiable AS negotiable, posts.condition AS condition, " +
            ...
            //     dbneo4j.cypher({ query: matchPosts }, (e, d) => {
            //         if (e) {
            //             responseObj = {
            //                 code: 500,
            //                 message: 'internal server error',
        ...
        async.waterfall([
            function sellingItems(cb) {
                var matchPosts = "MATCH (n:User {username : '" + username + "'})-[p:POSTS]->(posts) " +
                    ...
                    "OPTIONAL MATCH (user : User)-[allLikes : LIKES]->(posts) " +
                    "OPTIONAL MATCH (node3 : User)-[c : Commented]->(posts) " +
                    "WITH DISTINCT n, p, posts, l, user, allLikes, node3, c " +
                    "OPTIONAL MATCH (posts)<-[pr :inAppPurchase {status : 1}]-(promotionPlan :appPurchase) " +
                    "WITH DISTINCT pr.status AS isPromoted, promotionPlan.planId AS planId, n, p, posts, l, user, allLikes, node3, c " +
                    "OPTIONAL MATCH (posts)-[cat : category]->(category : Category) " +
                    "RETURN DISTINCT COUNT(l) AS likeStatus, ID(posts) AS postNodeId , labels(posts) AS label, posts.likes AS likes, posts.mainUrl AS mainUrl, " +
                    "posts.place AS place, posts.thumbnailImageUrl AS thumbnailImageUrl, toInt(posts.postId) AS postId, posts.hashTags as hashTags, " +
                    "posts.postCaption AS postCaption, posts.imageCount AS imageCount, isPromoted, planId, " +
    ...
    Router.post('/editProfile', function (req, res) {
        ...
        var matchQuery = 'MATCH (u:User {username:"' + username + '"}) ' +
            ...
            'u.businessName AS businessName, u.aboutBusiness AS aboutBusiness, u.mainBannerImageUrl AS mainBannerImageUrl, ' +
                    ...
                    if (err) {
                        ...
                        fs.readFile(filePath, {
                            ...
                            if (!err) {
                                ...
                                $('.customMessage').append(`We got a request to change your email address, Please click on the link to verify your new email address. <a href="${url}/verify-email/${req.body.token.trim()}/${randomString}">Click here </a> to verify your email id.`);
                                var mailData = {
                                    from: from_who,
                                    // to: data.email,
    ...
    Router.post('/saveProfile', function (req, res) {
        ...
        var username = req.decoded.name;
        var updateQuery = '';
        updateQuery = 'MATCH (u:User {username:"' + username + '"}) SET u.username = "' + username + '" ';


        if (username != req.body.username) {
            console.log("username not same", username);
            ...
            dbneo4j.cypher({
                ...
                if (error) {
                    ...
                    return res.send({
                     ...
                     updateQuery = 'MATCH (u:User {username:"' + username + '"}) SET u.username = "' + req.body.username.replace(/\s/g, '').toLowerCase() + '" ';
                }
            })
        }
        // else {
        ...
        if (req.body.place) updateQuery += ', u.place = ' + JSON.stringify(req.body.place.trim()) + ' ';
        ...
        updateQuery += 'RETURN ID(u) AS userId, u.username AS username, u.email AS email, u.profilePicUrl AS profilePicUrl, ' +
            ...
            'u.businessProfile AS businessProfile, u.businessName AS businessName, ' +
        ...
        var query = userCollection.findAndModify({
            ...
            function (e, d) {
                console.log("err-Mongo", e)
                console.log("data-Mongo", d)
                if (e) {
                    return res.send({
                        code: 500,
        ...
        async.waterfall([
            ...
            function memberProfile(data, cb) {
                var query = `MATCH (a : User {username : "` + membername + `"}), (x : User {username : "` + username + `"}) WITH a, x ` +
                    ...
                    `OPTIONAL MATCH (x)-[r:rating]->(a) WITH followers, following, posts, a, avg(r.rating) AS avgRating,followStatus,COUNT(r) AS ratedBy ` +
                    `RETURN DISTINCT a.phoneNumber AS phoneNumber, a.profilePicUrl AS profilePicUrl, a.fullName AS fullName,avgRating, ` +
                    `a.email AS email, a.bio AS bio, followers, following, posts, a.website AS website, a.username AS username,ratedBy, ` +
    ...
    Router.post('/profile/posts/:membername', (req, res) => {
        ...
        async.waterfall([
            ...
            function memberPosts(data, cb) {
                var findMemberPostsQuery = "MATCH (node : User {username : '" + membername + "'})-[p:POSTS]->(posts) " +
                    "WHERE (posts.banned <> 1 OR NOT EXISTS(posts.banned)) AND posts.sold = " + sold + "  " + soldCondition + " " +
                    "OPTIONAL MATCH (node2 : User {username : '" + username + "'})-[l:LIKES]-(posts)" +
                    "OPTIONAL MATCH (node3 : User)-[c : Commented]->(posts) " +
                    "OPTIONAL MATCH (posts)-[cat : category]->(category : Category) " +
                    "OPTIONAL MATCH (posts)-[pr :promotion]->(promotionPlan :promotionPlans) " +
                    ...
                    "COLLECT (DISTINCT {category : category.name, mainUrl : category.mainUrl, activeImageUrl : category.activeImageUrl}) AS categoryData " +
                    "ORDER BY (postId) DESC SKIP " + offset + " LIMIT " + limit + " ; ";
                // console.log(findMemberPostsQuery);
        ...
        async.waterfall([
            function memberProfile(cb) {
                var query = `MATCH (a : User {username : "` + membername + `"}) WITH a ` +
                    ...
                    `OPTIONAL MATCH (a)-[f1 : FOLLOWS]->(b : User) WHERE a <> b WITH DISTINCT COUNT(f1) AS following, posts, a ` +
                    `OPTIONAL MATCH (a)<-[f2 : FOLLOWS]-(c : User) WHERE a <> c WITH DISTINCT COUNT(f2) AS followers, following, posts, a ` +
                    `OPTIONAL MATCH (a)-[r:rating]->(x) WITH followers, following, posts, a, avg(r.rating) AS avgRating ` +
                    `RETURN DISTINCT a.phoneNumber AS phoneNumber, a.profilePicUrl AS profilePicUrl, a.fullName AS fullName,avgRating, ` +
    ...
    Router.post('/getPhotosOfYou', function (req, res) {
        ...
        var query = 'MATCH (node : User {username : "' + username + '"})-[t:Tagged]-(posts)<-[p : POSTS]-(node2 : User) ' +
            ...
            'ID(node2) AS postedByUserNodeId, node2.username AS postedByUserName, node2.profilePicUrl AS profilePicUrl, ' +
    ...
    Router.post('/getUserProfileBasics', function (req, res) {
        ...
        var query = 'OPTIONAL MATCH (a : User {username : "' + username + '"})-[p : POSTS]->(b) WITH COUNT(b) AS totalPosts ' +
            ...
            'OPTIONAL MATCH (a : User {username : "' + username + '"}) WITH a.username AS username, followerCount, followingCount, totalPosts,a.emailVerified AS emailVerified ' +
